...
def join_chat():
    ...
    broadcast_client_socket.sendto(encode_data({}), ("255.255.255.255", broadcast_port))
    print("join: chat: 1")
    
    data, address = broadcast_client_socket.recvfrom(buffer_size)
    print(decode_data(data))
    print("Chat joined successfully")
    broadcast_client_socket.close()
    # error handling if join chat did not work --> leader is missing
    try:
        s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        s.bind((IPAddr, 15000))
        s.listen()
        ...
        x = True
        while x :
            d, a = s.recv(buffer_size)
            print("id: ", decode_data(d).id)
    finally:
        s.close
...
def listen_for_participants():
    ...
    while True:
        try:
            ...
            broadcast_server_socket.se
            new_participant = decode_data(new_participant)

            s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            s.connect((address, 15000))
            s.send(encode_data({"id": str(uuid4())}))
            s.close()
